...
const CourseForm = ({ onClose, onSuccess, editData = null }) => {
  ...
  useEffect(() => {
    const handler = setTimeout(async () => {
      if (selectedCountry && selectedUniversityId && selectedCourseName) {
        ...
        try {
          ...
          setValue(
            "masters_requirement",
            fetchedData.masters_requirement || ""
          );
          ...
          setValue("website_url", fetchedData.website_url || "");
          ...
          toast.error(
            "Failed to fetch course details. Please enter manually.",
            { id: "fetching-course-info" }
          );
        } finally {
    ...
    return () => {
      ...
      toast.dismiss("fetching-course-info");
    };
  }, [
    selectedCountry,
    selectedUniversityId,
