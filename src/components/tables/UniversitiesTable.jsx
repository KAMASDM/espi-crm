      ...
      <div className="table-container">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th onClick={() => handleSort('univ_name')} className="table-header cursor-pointer hover:bg-gray-100">
                <div className="flex items-center">University Name{sortField === 'univ_name' && (<span className="ml-1">{sortDirection === 'asc' ? '↑' : '↓'}</span>)}</div>
              </th>
              <th onClick={() => handleSort('country')} className="table-header cursor-pointer hover:bg-gray-100">
                <div className="flex items-center">Country{sortField === 'country' && (<span className="ml-1">{sortDirection === 'asc' ? '↑' : '↓'}</span>)}</div>
              </th>
              ...
              <th onClick={() => handleSort('createdAt')} className="table-header cursor-pointer hover:bg-gray-100">
                <div className="flex items-center">Created{sortField === 'createdAt' && (<span className="ml-1">{sortDirection === 'asc' ? '↑' : '↓'}</span>)}</div>
              </th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredUniversities.length === 0 ? (
              ...
              filteredUniversities.map((university) => {
                ...
                if (university.createdAt && typeof university.createdAt.toDate === 'function') {
                  try { formattedDate = format(university.createdAt.toDate(), 'MMM dd, yyyy'); }
                  catch (e) { formattedDate = 'Invalid Date'; }
                } else if (university.createdAt) {
                  try {
                      const parsedDate = new Date(university.createdAt);
                      if (!isNaN(parsedDate)) { formattedDate = format(parsedDate, 'MMM dd, yyyy');}
                      else {formattedDate = 'Invalid Date';}
                  ...
                  <tr key={university.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">
                      <div className="space-y-1">
                        ...
                        {university.univ_email && (<div className="flex items-center text-sm text-gray-500"><Mail size={14} className="mr-2 text-gray-400" />{university.univ_email}</div>)}
