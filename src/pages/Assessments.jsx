...
import { Plus, Download, Upload, ClipboardList } from 'lucide-react';
...
const Assessments = () => {
  ...
  const handleFormSuccess = () => {
    // The useAssessments hook will automatically update the data
  };
  ...
  // Calculate stats
  const pendingAssessments = assessments.filter(a => a.ass_status === 'Pending').length;
  const inProgressAssessments = assessments.filter(a => a.ass_status === 'In Progress').length;
  const completedAssessments = assessments.filter(a => a.ass_status === 'Completed').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Evaluate student profiles and recommend suitable programs</p>
        </div>
...
// Assessment Details Component for View Modal
const AssessmentDetails = ({ assessment, enquiries, universities, courses }) => {
  const getStudentName = (enquiryId) => {
    const enquiry = enquiries.find(enq => enq.id === enquiryId);
    return enquiry ? `${enquiry.student_First_Name} ${enquiry.student_Last_Name}` : 'Unknown Student';
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-900">{getStudentName(assessment.enquiry)}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Student Email</label>
            <p className="text-sm text-gray-900">{getStudentEmail(assessment.enquiry)}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Country of Interest</label>
            <p className="text-sm text-gray-900">{getCountryName(assessment.student_country)}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Level Applying For</label>
            <p className="text-sm text-gray-900">{assessment.level_applying_for}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        <h4 className="text-lg font-semibold text-gray-900 mb-3">University & Course Information</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">University</label>
            <p className="text-sm text-gray-900">{getUniversityName(assessment.university)}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Course</label>
            <p className="text-sm text-gray-900">{getCourseName(assessment.course_interested)}</p>
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Intake Interested</label>
            <p className="text-sm text-gray-900">{assessment.intake_interested || 'Not specified'}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Course Link</label>
            <p className="text-sm text-gray-900">
              {assessment.course_link ? (
                <a href={assessment.course_link} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:text-primary-700">
                  {assessment.course_link}
                </a>
              ) : 'Not provided'}
            </p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-900">{assessment.fee_currency || 'Not specified'}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        <h4 className="text-lg font-semibold text-gray-900 mb-3">Assessment Status & Notes</h4>
            ...
            <p className="text-sm text-gray-900">
              {assessment.ass_status ? (
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  assessment.ass_status === 'Completed' ? 'bg-green-100 text-green-800' :
                  assessment.ass_status === 'In Progress' ? 'bg-blue-100 text-blue-800' :
                  assessment.ass_status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
