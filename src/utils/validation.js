...
export const phone = (value) => {
  ...
  return phoneRegex.test(cleanPhone) ? null : 'Please enter a valid 10-digit phone number';
};
...
export const studentName = (value) => {
  ...
  return nameRegex.test(value) ? null : 'Name can only contain letters, spaces, hyphens, and apostrophes';
};
...
export const percentage = (value) => {
  ...
  return (numValue >= 0 && numValue <= 100) ? null : 'Percentage must be between 0 and 100';
};
...
export const gpa = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 0 && numValue <= 4.0) ? null : 'GPA must be between 0 and 4.0';
};
...
export const ieltsScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 0 && numValue <= 9.0) ? null : 'IELTS score must be between 0 and 9.0';
};
...
export const toeflScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 0 && numValue <= 120) ? null : 'TOEFL score must be between 0 and 120';
};
...
export const greScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 130 && numValue <= 170) ? null : 'GRE score must be between 130 and 170';
};
...
export const gmatScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 200 && numValue <= 800) ? null : 'GMAT score must be between 200 and 800';
};
...
export const pteScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 10 && numValue <= 90) ? null : 'PTE score must be between 10 and 90';
};
...
export const duolingoScore = (value) => {
  ...
  const numValue = parseFloat(value);
  if (isNaN(numValue)) return 'Must be a valid number';
  return (numValue >= 10 && numValue <= 160) ? null : 'Duolingo score must be between 10 and 160';
};
...
export const yearOfPassing = (value) => {
  ...
  return (numValue >= 1950 && numValue <= currentYear + 5) ? null : `Year must be between 1950 and ${currentYear + 5}`;
};
...
export const fileType = (allowedTypes) => (file) => {
  ...
  return isAllowed ? null : `File type must be one of: ${allowedTypes.join(', ')}`;
};
...
export const validateEnquiryForm = (data) => {
  ...
  if (!data.student_First_Name) errors.student_First_Name = required(data.student_First_Name);
  else errors.student_First_Name = studentName(data.student_First_Name);
  
  if (!data.student_Last_Name) errors.student_Last_Name = required(data.student_Last_Name);
  else errors.student_Last_Name = studentName(data.student_Last_Name);
  
  errors.student_phone = required(data.student_phone) || phone(data.student_phone);
  errors.student_email = required(data.student_email) || email(data.student_email);
...
export const validateUniversityForm = (data) => {
  ...
  if (data.Application_fee) errors.Application_fee = applicationFee(data.Application_fee);
  if (data.Backlogs_allowed) errors.Backlogs_allowed = integer(data.Backlogs_allowed);
...
export const validateCourseForm = (data) => {
  ...
  if (data.Application_fee) errors.Application_fee = applicationFee(data.Application_fee);
  if (data.Yearly_Tuition_fee) errors.Yearly_Tuition_fee = tuitionFee(data.Yearly_Tuition_fee);
  if (data.Backlogs_allowed) errors.Backlogs_allowed = integer(data.Backlogs_allowed);
...
export const validatePaymentForm = (data) => {
  ...
  errors.payment_amount = required(data.payment_amount) || positiveNumber(data.payment_amount);
